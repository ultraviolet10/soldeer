...
fn check_clean_success(dir: &Path, config_filename: &str) {
    ...
    // Verify config still contains dependencies
...
/// Validation helper to check that artifacts exist before cleaning
#[allow(clippy::unwrap_used)]
...
async fn test_clean_foundry_config() {
    ...
    // Verify dependencies and lock file exist before cleaning
    check_artifacts_exist(&dir);

    // Run clean command using builder pattern
    let cmd: Command = Clean::builder().build().into();
    let res = async_with_vars(
        [("SOLDEER_PROJECT_ROOT", Some(dir.to_string_lossy().as_ref()))],
        run(cmd, Verbosity::default()),
...
async fn test_clean_restores_with_install() {
    ...
    // Check that dependencies were actually installed
...
async fn test_clean_with_complex_file_structure() {
    ...
    // Create complex nested structure in dependencies
...
async fn test_clean_permission_error() {
    ...
    // Make dependencies folder read-only (Unix only)
    #[cfg(unix)]
    {
...
async fn test_clean_partial_failure() {
    ...
    // Remove lock file manually to simulate partial state
    fs::remove_file(dir.join("soldeer.lock")).unwrap();

    // Clean should still succeed (only dependencies folder exists)
    let cmd: Command = Clean::builder().build().into();
    let res = async_with_vars(
        [("SOLDEER_PROJECT_ROOT", Some(dir.to_string_lossy().as_ref()))],
        run(cmd, Verbosity::default()),
...
async fn test_clean_with_soldeer_config_variations() {
    ...
    // Test with custom soldeer configuration
...
async fn test_clean_multiple_times() {
    ...
    // Second clean (should be idempotent)
    let cmd: Command = Clean::builder().build().into();
    let res = async_with_vars(
        [("SOLDEER_PROJECT_ROOT", Some(dir.to_string_lossy().as_ref()))],
        run(cmd, Verbosity::default()),
